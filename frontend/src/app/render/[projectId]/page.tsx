...
import { CameraAnglePicker } from '@/components/render/CameraAnglePicker';
import type { SavedCameraAngle } from '@/components/render/CameraAnglePicker';
import {
  ImageIcon,
  CheckCircle2,
  Clock,
...
const ROOM_ANGLES: Record<string, string[]> = {
  ...
  bed2: ['Door View', 'Window-side', 'Desk Corner'],
};
...
export default function RenderGalleryPage() {
  ...
  const [savedAngles, setSavedAngles] = useState<SavedCameraAngle[]>([]);
  const [cameraPickerRoom, setCameraPickerRoom] = useState<string | null>(null);
  ...
  // ── Handle custom camera angles ──

  const handleSaveAngle = useCallback((angle: SavedCameraAngle) => {
    setSavedAngles((prev) => [...prev, angle]);
  }, []);
  ...
  // Combine auto + custom angles for selected room display
  const getAllRoomAngles = useCallback(
    (roomType: string): { name: string; isCustom: boolean }[] => {
      const auto = (ROOM_ANGLES[roomType] || []).map((name) => ({
        name,
        isCustom: false as const,
      }));
      const custom = savedAngles
        .filter((a) => a.roomType === roomType && !a.isAutoGenerated)
        .map((a) => ({ name: a.name, isCustom: true as const }));
      return [...auto, ...custom];
    },
    [savedAngles],
  );
      ...
      <div className="max-w-4xl mx-auto w-full px-6 py-8 overflow-y-auto">
        ...
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {ROOMS.map((r) => {
              const angles = getAllRoomAngles(r.roomType);
              const hasRender = renders.some((rend) => rend.roomType === r.roomType);
              const customCount = savedAngles.filter(
                (a) => a.roomType === r.roomType && !a.isAutoGenerated,
              ).length;
              return (
                <div key={r.roomType} className="bg-slate-700/20 p-3 rounded-lg">
                  ...
                  <div className="text-[9px] text-slate-500 uppercase tracking-wider mb-1 mt-2">
                    Angles: Auto
                  </div>
                  <div className="space-y-1 text-[10px] text-slate-400">
                    {ROOM_ANGLES[r.roomType]?.map((angle) => (
                      <label
                        key={angle}
                  ...
                  {customCount > 0 && (
                    <>
                      ...
                      <div className="space-y-1 text-[10px] text-slate-400">
                        {savedAngles
                          .filter((a) => a.roomType === r.roomType && !a.isAutoGenerated)
                          .map((angle) => (
                            <label
                              key={angle.id}
                              className="flex items-center gap-1.5 cursor-pointer"
                            >
                              <input
                                type="checkbox"
                                defaultChecked={selectedAngles[r.roomType]?.includes(angle.name)}
                                disabled={!sampleGenerated || isGenerating}
                                className="accent-teal-500"
                                onChange={() => {
                                  const current = selectedAngles[r.roomType] || [];
                                  if (current.includes(angle.name)) {
                                    ...
                                    selectedAngles[r.roomType] = [...current, angle.name];
                                  }
                                }}
                              />
                  ...
                  <button
                    onClick={() => setCameraPickerRoom(r.roomType)}
                    disabled={!sampleGenerated || isGenerating}
                    className="mt-2 w-full text-left text-[10px] text-teal-400 hover:text-teal-300 transition-colors flex items-center gap-1 disabled:opacity-40"
                  >
                  ...
                  {hasRender && (
                    <Button
                      variant="ghost"
                      size="sm"
                      className="text-teal-400 text-[10px] mt-1 p-0 h-auto"
                      onClick={() => {
